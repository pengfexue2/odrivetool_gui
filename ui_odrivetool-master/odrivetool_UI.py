...
class ExampleApp(QtWidgets.QMainWindow, UI_mainwindow.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.pushButton_clearError.clicked.connect(self.clear_error)
        ...
        self.axis0TrajectoryMove_pushButton.clicked.connect(self.send_axis0_pads_go)
        ...
        self.axis1TrajectoryMove_pushButton.clicked.connect(self.send_axis1_pads_go)
        ...
        self.axis0_pos_gain_add_pb.clicked.connect(self.add_axis0_pos_gain)
        self.axis0_pos_gain_min_pb.clicked.connect(self.min_axis0_pos_gain)

        self.axis0_vel_gain_add_pb.clicked.connect(self.add_axis0_vel_gain)
        self.axis0_vel_gain_min_pb.clicked.connect(self.min_axis0_vel_gain)

        self.axis0_vel_int_add_pb.clicked.connect(self.add_axis0_vel_int)
        self.axis0_vel_int_min_pb.clicked.connect(self.min_axis0_vel_int)

        self.axis1_pos_gain_add_pb.clicked.connect(self.add_axis1_pos_gain)
        self.axis1_pos_gain_min_pb.clicked.connect(self.min_axis1_pos_gain)

        self.axis1_vel_gain_add_pb.clicked.connect(self.add_axis1_vel_gain)
        self.axis1_vel_gain_min_pb.clicked.connect(self.min_axis1_vel_gain)

        self.axis1_vel_int_add_pb.clicked.connect(self.add_axis1_vel_int)
        self.axis1_vel_int_min_pb.clicked.connect(self.min_axis1_vel_int)

        self.boards_comboBox.currentIndexChanged.connect(self.exchange_odrive)
        ...
        self.config_url = "ODrives.ini"
        self.odrive_config = self.read_config()
        self.odrive_list = list()
    ...
    def add_axis0_pos_gain(self):
        self.my_drive.axis0.controller.config.pos_gain *= 1.3
        self.doubleSpinBox_posGainValue_axis0.setValue(self.my_drive.axis0.controller.config.pos_gain)
    def min_axis0_pos_gain(self):
        self.my_drive.axis0.controller.config.pos_gain *= 0.5
        self.doubleSpinBox_posGainValue_axis0.setValue(self.my_drive.axis0.controller.config.pos_gain)
    def add_axis0_vel_gain(self):
        self.my_drive.axis0.controller.config.vel_gain *= 1.3
        self.doubleSpinBox_velGainValue_axis0.setValue(self.my_drive.axis0.controller.config.vel_gain)
    def min_axis0_vel_gain(self):
        self.my_drive.axis0.controller.config.vel_gain *= 0.5
        self.doubleSpinBox_velGainValue_axis0.setValue(self.my_drive.axis0.controller.config.vel_gain)
    def add_axis0_vel_int(self):
        self.my_drive.axis0.controller.config.vel_integrator_gain *= 1.3
        self.doubleSpinBox_velIntegratorGainValue_axis0.setValue(
            self.my_drive.axis0.controller.config.vel_integrator_gain)
    def min_axis0_vel_int(self):
        self.my_drive.axis0.controller.config.vel_integrator_gain *= 0.5
        self.doubleSpinBox_velIntegratorGainValue_axis0.setValue(
            self.my_drive.axis0.controller.config.vel_integrator_gain)

    def add_axis1_pos_gain(self):
        self.my_drive.axis1.controller.config.pos_gain *= 1.3
        self.doubleSpinBox_posGainValue_axis1.setValue(self.my_drive.axis1.controller.config.pos_gain)
    def min_axis1_pos_gain(self):
        self.my_drive.axis1.controller.config.pos_gain *= 0.5
        self.doubleSpinBox_posGainValue_axis1.setValue(self.my_drive.axis1.controller.config.pos_gain)
    def add_axis1_vel_gain(self):
        self.my_drive.axis1.controller.config.vel_gain *= 1.3
        self.doubleSpinBox_velGainValue_axis1.setValue(self.my_drive.axis1.controller.config.vel_gain)
    def min_axis1_vel_gain(self):
        self.my_drive.axis1.controller.config.vel_gain *= 0.5
        self.doubleSpinBox_velGainValue_axis1.setValue(self.my_drive.axis1.controller.config.vel_gain)
    def add_axis1_vel_int(self):
        self.my_drive.axis1.controller.config.vel_integrator_gain *= 1.3
        self.doubleSpinBox_velIntegratorGainValue_axis1.setValue(
            self.my_drive.axis1.controller.config.vel_integrator_gain)
    def min_axis1_vel_int(self):
        self.my_drive.axis1.controller.config.vel_integrator_gain *= 0.5
        self.doubleSpinBox_velIntegratorGainValue_axis1.setValue(
            self.my_drive.axis1.controller.config.vel_integrator_gain)
    ...
    def update_controller_mode(self):
        ...
        if axis0_control_mode == CTRL_MODE_POSITION_CONTROL:
            if self.axis0Trajectory_radioButton.isChecked():
                pass
            else:
    ...
    def axis_controller_mode_changed(self, id):
        ...
        if group_name == "buttonGroup_13":
            ...
            elif button_name == "TraJectory":
                self.axis_control_mode_changed(CTRL_MODE_TRAJECTORY_CONTROL, 0)
    ...
    def axis_control_mode_changed(self, control_mode, axis):
        if control_mode == CTRL_MODE_POSITION_CONTROL:
            ...
            self.axis_controller_fields_position_enabled(False, axis)
        elif control_mode == CTRL_MODE_TRAJECTORY_CONTROL:
            self.axis_controller_fields_position_enabled(True, axis)
    ...
    def axis_controller_fields_position_enabled(self, state, axis):
        if axis == 0:
            ...
            self.axis0TrajectoryAcc_doubleSpinBox.setEnabled(state)
            self.axis0TrajectoryDec_doubleSpinBox.setEnabled(state)
            self.axis0TrajectoryPos_doubleSpinBox.setEnabled(state)
            self.axis0TrajectoryMove_pushButton.setEnabled(state)
            self.axis0TrajectorySpeed_doubleSpinBox.setEnabled(state)
            ...
            self.axis1TrajectoryAcc_doubleSpinBox.setEnabled(state)
            self.axis1TrajectoryDec_doubleSpinBox.setEnabled(state)
            self.axis1TrajectoryPos_doubleSpinBox.setEnabled(state)
            self.axis1TrajectoryMove_pushButton.setEnabled(state)
            self.axis1TrajectorySpeed_doubleSpinBox.setEnabled(state)
    ...
    def read_config(self):
        odrive_sn_ini = ConfigParser()
        odrive_sn_ini.read(self.config_url)
        id_list = odrive_sn_ini.items(section="ODRIVE_DEVICE")
        print("~"*10)
        print(id_list)
        print("~" * 10)
        return id_list
    ...
    def odrive_connect(self):
        thread = Thread(target=self.load_odrives)
        thread.start()

        self.pushButton_connect.setDisabled(True)
        self.pushButton_connect.setText("Connecting . . .")

    def fetch_odrive(self, odrive_label, odrive_sn):
        try:
            temp_odrive = odrive.find_any(serial_number=odrive_sn, timeout=20)
            if temp_odrive is not None:
                self.odrive_list.append(temp_odrive)
                self.odrive_dict[odrive_label] = temp_odrive
        except:
            pass

    def load_odrives(self):
        print("odrives loading...")
        self.odrive_dict = dict()
        for odrive_label, odrive_sn in self.odrive_config:
            temp_thread = Thread(target=lambda: self.fetch_odrive(odrive_label, odrive_sn))
            self.thread_list.append(temp_thread)
            temp_thread.start()
        ...
        print("odrives loaded!!!")
        self.boards_comboBox.clear()
        self.boards_comboBox.addItems(self.odrive_dict.keys())
        print(self.odrive_dict)
        self.odrive_worker = ODriveWorker(self.odrive_list[0])
    ...
    def exchange_odrive(self, index):
        print(index)
        self.timer.stop()
        self.timer_graphUpdate.stop()
        self.odrive_worker.stop()
        ...
        self.odrive_worker = ODriveWorker(self.odrive_list[index])
        self.odrive_worker.odrive_found_sig.connect(self.odrive_connected)
        self.odrive_worker.start()
        print("!!!")
    ...
    def setDisabled_odrive_ui(self, state):
        ...
        self.pushButton_clearError.setDisabled(state)
    ...
    def update_velocity_graph(self, axis_key, odrv_axis):
        ...
        self.ad[axis_key]["vel_sp_curve"].setData(self.ad[axis_key]["time_array"],
                                                  self.ad[axis_key]["velocity"]["set_point"])
        self.ad[axis_key]["vel_est_curve"].setData(self.ad[axis_key]["time_array"],
                                                   self.ad[axis_key]["velocity"]["estimate"])
    ...
    def update_current_graph(self, axis_key, odrv_axis):
        ...
        self.ad[axis_key]["current_sp_curve"].setData(self.ad[axis_key]["time_array"],
                                                      self.ad[axis_key]["current"]["set_point"])
        self.ad[axis_key]["current_est_curve"].setData(self.ad[axis_key]["time_array"],
                                                       self.ad[axis_key]["current"]["estimate"])
    ...
    def update_position_graph(self, axis_key, odrv_axis):
        ...
        self.ad[axis_key]["pos_sp_curve"].setData(self.ad[axis_key]["time_array"],
                                                  self.ad[axis_key]["position"]["set_point"])
        self.ad[axis_key]["pos_est_curve"].setData(self.ad[axis_key]["time_array"],
                                                   self.ad[axis_key]["position"]["estimate"])
    ...
    def clear_error(self):
        dump_errors(self.my_drive, True)
    ...
    def write_config(self):
        ...
        self.my_drive.axis0.motor.config.calibration_current = float(
            self.doubleSpinBox_calibrationCurrentValue_axis0.value())
        ...
        self.my_drive.axis0.motor.config.requested_current_range = float(
            self.doubleSpinBox_requestedCurrentRangeValue_axis0.value())
        # self.my_drive.axis0.motor.config.current_control_bandwidth = float(self.doubleSpinBox_currentControlBandwidthValue.value())
        self.my_drive.axis0.motor.config.resistance_calib_max_voltage = float(
            self.doubleSpinBox_resistanceCalibMaxVoltageValue_axis0.value())
        ...
        self.my_drive.axis0.controller.config.vel_integrator_gain = float(
            self.doubleSpinBox_velIntegratorGainValue_axis0.value())
        ...
        self.my_drive.axis1.motor.config.calibration_current = float(
            self.doubleSpinBox_calibrationCurrentValue_axis1.value())
        ...
        self.my_drive.axis1.motor.config.requested_current_range = float(
            self.doubleSpinBox_requestedCurrentRangeValue_axis1.value())
        # self.my_drive.axis1.motor.config.current_control_bandwidth = float(self.doubleSpinBox_currentControlBandwidthValue.value())
        self.my_drive.axis1.motor.config.resistance_calib_max_voltage = float(
            self.doubleSpinBox_resistanceCalibMaxVoltageValue_axis1.value())
        ...
        self.my_drive.axis1.controller.config.vel_integrator_gain = float(
            self.doubleSpinBox_velIntegratorGainValue_axis1.value())
        ...
        self.my_drive.config.dc_bus_overvoltage_trip_level = float(
            self.doubleSpinBox_dcBusOvervoltageTripLevelValue.value())
        self.my_drive.config.enable_ascii_protocol_on_usb = self.radioButton_enableAsciiProtocolOnUsbTrue.isChecked()
        self.my_drive.config.dc_bus_undervoltage_trip_level = float(
            self.doubleSpinBox_dcBusUndervoltageTripLevelValue.value())
    ...
    def scan_low_level_config(self):
        ...
        self.label_hwVersionValue.setText(
            "{}.{}".format(self.my_drive.hw_version_major, self.my_drive.hw_version_minor))
        self.label_hwVersionVariantValue.setText(str(self.my_drive.hw_version_variant))
        self.label_fwVersionValue.setText(
            "{}.{}".format(self.my_drive.fw_version_major, self.my_drive.fw_version_minor))
    ...
    def scan_axis0_motor_config(self):
        ...
        self.scan_axis0_trap_traj_config()

    def scan_axis0_trap_traj_config(self):
        self.axis0TrajectorySpeed_doubleSpinBox.setValue(self.my_drive.axis0.trap_traj.config.vel_limit)
        self.axis0TrajectoryAcc_doubleSpinBox.setValue(self.my_drive.axis0.trap_traj.config.accel_limit)
        self.axis0TrajectoryDec_doubleSpinBox.setValue(self.my_drive.axis0.trap_traj.config.decel_limit)


    def scan_axis0_motor_currentControl_config(self):
        self.label_maxAllowedCurrentValue_axis0.setText(
            str(self.my_drive.axis0.motor.current_control.max_allowed_current))
        self.label_ibusValue_axis0.setText(str(self.my_drive.axis0.motor.current_control.Ibus))
        self.label_vCurrentControlIntegralDValue_axis0.setText(
            str(self.my_drive.axis0.motor.current_control.v_current_control_integral_d))
        self.label_vCurrentControlIntegralQValue_axis0.setText(
            str(self.my_drive.axis0.motor.current_control.v_current_control_integral_q))
    ...
    def scan_axis0_motor_config_config(self):
        ...
        self.doubleSpinBox_currentControlBandwidthValue_axis0.setValue(
            self.my_drive.axis0.motor.config.current_control_bandwidth)
        self.doubleSpinBox_requestedCurrentRangeValue_axis0.setValue(
            self.my_drive.axis0.motor.config.requested_current_range)
        self.spinBox_directionValue_axis0.setValue(self.my_drive.axis0.motor.config.direction)
        self.doubleSpinBox_resistanceCalibMaxVoltageValue_axis0.setValue(
            self.my_drive.axis0.motor.config.resistance_calib_max_voltage)
    ...
    def scan_axis1_motor_config(self):
        ...
        self.scan_axis1_trap_traj_config()

    def scan_axis1_trap_traj_config(self):
        self.axis1TrajectorySpeed_doubleSpinBox.setValue(self.my_drive.axis0.trap_traj.config.vel_limit)
        self.axis1TrajectoryAcc_doubleSpinBox.setValue(self.my_drive.axis0.trap_traj.config.accel_limit)
        self.axis1TrajectoryDec_doubleSpinBox.setValue(self.my_drive.axis0.trap_traj.config.decel_limit)

    def scan_axis1_motor_currentControl_config(self):
        self.label_maxAllowedCurrentValue_axis1.setText(
            str(self.my_drive.axis1.motor.current_control.max_allowed_current))
        self.label_ibusValue_axis1.setText(str(self.my_drive.axis1.motor.current_control.Ibus))
        self.label_vCurrentControlIntegralDValue_axis1.setText(
            str(self.my_drive.axis1.motor.current_control.v_current_control_integral_d))
        self.label_vCurrentControlIntegralQValue_axis1.setText(
            str(self.my_drive.axis1.motor.current_control.v_current_control_integral_q))
    ...
    def scan_axis1_motor_config_config(self):
        ...
        self.doubleSpinBox_currentControlBandwidthValue_axis1.setValue(
            self.my_drive.axis1.motor.config.current_control_bandwidth)
        self.doubleSpinBox_requestedCurrentRangeValue_axis1.setValue(
            self.my_drive.axis1.motor.config.requested_current_range)
        self.spinBox_directionValue_axis1.setValue(self.my_drive.axis1.motor.config.direction)
        self.doubleSpinBox_resistanceCalibMaxVoltageValue_axis1.setValue(
            self.my_drive.axis1.motor.config.resistance_calib_max_voltage)
    ...
    def send_axis0_pads_go(self):
        speed = self.axis0TrajectorySpeed_doubleSpinBox.value()
        acc = self.axis0TrajectoryAcc_doubleSpinBox.value()
        dec = self.axis0TrajectoryDec_doubleSpinBox.value()
        if speed > 0:
            self.my_drive.axis0.trap_traj.config.vel_limit = speed
        if acc > 0:
            self.my_drive.axis0.trap_traj.config.accel_limit = acc
        if dec > 0:
            self.my_drive.axis0.trap_traj.config.decel_limit = dec
        self.my_drive.axis0.controller.move_to_pos(self.axis0TrajectoryPos_doubleSpinBox.value())

    def send_axis1_pads_go(self):
        speed = self.axis1TrajectorySpeed_doubleSpinBox.value()
        acc = self.axis1TrajectoryAcc_doubleSpinBox.value()
        dec = self.axis1TrajectoryDec_doubleSpinBox.value()
        if speed > 0:
            self.my_drive.axis1.trap_traj.config.vel_limit = speed
        if acc > 0:
            self.my_drive.axis1.trap_traj.config.accel_limit = acc
        if dec > 0:
            self.my_drive.axis1.trap_traj.config.decel_limit = dec
        self.my_drive.axis1.controller.move_to_pos(self.axis1TrajectoryPos_doubleSpinBox.value())
